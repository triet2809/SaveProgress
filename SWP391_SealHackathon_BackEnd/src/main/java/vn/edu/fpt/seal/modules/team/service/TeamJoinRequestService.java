...
public class TeamJoinRequestService {
    ...
    public JoinRequestResponse create(CreateJoinRequestRequest req, Authentication auth) {
        ...
        if (caller.getStatus() != AccountStatus.approved) throw ApiException.badRequest("Only approved users can request to join teams");
        if (teamMemberRepository.existsByTeamIdAndUserId(team.getId(), callerId)) throw ApiException.conflict("You are already a member of this team");
        if (teamMemberRepository.countByTeamId(team.getId()) >= MAX_TEAM_SIZE) throw ApiException.badRequest("This team is already full");
        if (requestRepository.existsByTeamIdAndUserIdAndStatus(team.getId(), callerId, "pending")) throw ApiException.conflict("You already have a pending request for this team");
    ...
    public void cancel(UUID id, Authentication auth) {
        TeamJoinRequest r = requestRepository.findWithRelationsById(id).orElseThrow(() -> ApiException.notFound("Join request not found: " + id));
        if (!r.getUser().getId().equals(currentUserId(auth))) throw ApiException.forbidden("You can only cancel your own request");
        if (!"pending".equals(r.getStatus())) throw ApiException.badRequest("Request is not pending");
        r.setStatus("cancelled"); r.setRespondedAt(LocalDateTime.now());
    }
    ...
    private void ensureLeaderOrCoordinator(UUID teamId, Authentication auth) {
        ...
        if (m.getRole() != TeamMemberRole.leader) throw ApiException.forbidden("Only the team leader can manage join requests");
    }
