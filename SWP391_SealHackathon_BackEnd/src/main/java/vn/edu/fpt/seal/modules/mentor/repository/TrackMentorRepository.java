...
public interface TrackMentorRepository extends JpaRepository<TrackMentor, UUID> {
    ...
    @EntityGraph(attributePaths = {"event", "track", "user"}) Page<TrackMentor> findByEventId(UUID eventId, Pageable pageable);
    @EntityGraph(attributePaths = {"event", "track", "user"}) Page<TrackMentor> findByEventIdAndTrackId(UUID eventId, UUID trackId, Pageable pageable);
    @EntityGraph(attributePaths = {"event", "track", "user"}) Page<TrackMentor> findByEventIdAndUserId(UUID eventId, UUID userId, Pageable pageable);
    @EntityGraph(attributePaths = {"event", "track", "user"}) List<TrackMentor> findAllByEventId(UUID eventId);
    ...
    interface MentorTeamRow {
        UUID getTrackMentorId(); UUID getMentorId(); UUID getEventId(); String getEventName(); UUID getTrackId(); String getTrackName(); UUID getRoundId(); String getRoundName(); UUID getTeamId(); String getTeamName(); TeamStatus getTeamStatus();
    }
}
