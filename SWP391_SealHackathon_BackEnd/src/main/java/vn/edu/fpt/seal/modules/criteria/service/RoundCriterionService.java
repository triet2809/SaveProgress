...
public class RoundCriterionService {
    ...
    public Page<RoundCriterionResponse> list(UUID eventId, UUID roundId, UUID trackId, Pageable pageable) {
     ...
     */
    @Transactional(readOnly = true)
    public RoundCriterionResponse get(UUID id) {
        return RoundCriterionMapper.toResponse(findOrThrow(id));
    }
     ...
     */
    @Transactional
    public RoundCriterionResponse create(CreateRoundCriterionRequest req) {
        Round round = roundRepository.findById(req.roundId()).orElseThrow(() -> ApiException.notFound("Round not found: " + req.roundId()));
        ensureEditable(round);
        String name = req.name().trim();
        if (criterionRepository.existsByRoundIdAndNameIgnoreCase(round.getId(), name))
            throw ApiException.conflict("Criterion name already exists in this round");
     ...
     */
    @Transactional
    public RoundCriterionResponse update(UUID id, UpdateRoundCriterionRequest req) {
        RoundCriterion c = findOrThrow(id);
        ensureEditable(c.getRound());
        if (req.name() != null) {
            String name = req.name().trim();
            if (!name.equalsIgnoreCase(c.getName()) && criterionRepository.existsByRoundIdAndNameIgnoreCase(c.getRound().getId(), name))
                throw ApiException.conflict("Criterion name already exists in this round");
            c.setName(name);
        }
        if (req.weight() != null) c.setWeight(req.weight());
        if (req.description() != null) c.setDescription(trim(req.description()));
        if (req.status() != null) c.setStatus(req.status().trim());
        return RoundCriterionMapper.toResponse(c);
    }
     ...
     */
    private RoundCriterion findOrThrow(UUID id) {
        return criterionRepository.findWithRoundById(id).orElseThrow(() -> ApiException.notFound("Criterion not found: " + id));
    }

    /**
     * Đảm bảo vòng thi còn chỉnh sửa được: chặn khi sự kiện đã completed hoặc cancelled.
     */
    private void ensureEditable(Round round) {
        EventStatus s = round.getTrack().getEvent().getStatus();
        if (s == EventStatus.completed || s == EventStatus.cancelled)
            throw ApiException.badRequest("Cannot edit criteria in event status " + s);
    }
