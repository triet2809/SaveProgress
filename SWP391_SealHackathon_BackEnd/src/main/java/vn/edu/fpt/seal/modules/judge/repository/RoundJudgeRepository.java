...
public interface RoundJudgeRepository extends JpaRepository<RoundJudge, UUID> {
    ...
    @EntityGraph(attributePaths = {"round", "round.track", "round.track.event", "user"}) Page<RoundJudge> findByRoundTrackEventId(UUID eventId, Pageable pageable);
    @EntityGraph(attributePaths = {"round", "round.track", "round.track.event", "user"}) Page<RoundJudge> findByRoundTrackEventIdAndRoundId(UUID eventId, UUID roundId, Pageable pageable);
    @EntityGraph(attributePaths = {"round", "round.track", "round.track.event", "user"}) Page<RoundJudge> findByRoundTrackEventIdAndUserId(UUID eventId, UUID userId, Pageable pageable);
    @EntityGraph(attributePaths = {"round", "round.track", "round.track.event", "user"}) List<RoundJudge> findAllByRoundTrackEventId(UUID eventId);
    ...
    interface JudgeSubmissionRow {
        UUID getRoundJudgeId(); UUID getJudgeId(); UUID getEventId(); String getEventName(); UUID getTrackId(); String getTrackName();
        UUID getRoundId(); String getRoundName(); UUID getTeamId(); String getTeamName(); UUID getSubmissionId();
        String getRepoUrl(); String getPresentationUrl(); String getDemoUrl(); String getStatus(); String getReviewStatus(); LocalDateTime getSubmittedAt();
    }
}
