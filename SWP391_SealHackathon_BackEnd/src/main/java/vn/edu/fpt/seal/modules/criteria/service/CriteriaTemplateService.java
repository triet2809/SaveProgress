...
public class CriteriaTemplateService {
    ...
    public CriteriaTemplateResponse get(UUID id) {
        return RoundCriterionMapper.toTemplateResponse(find(id));
    }
    ...
    public CriteriaTemplateResponse create(CreateCriteriaTemplateRequest req) {
        ...
        if (repository.existsByNameIgnoreCase(name))
            throw ApiException.conflict("Criteria template name already exists");
    ...
    public CriteriaTemplateResponse update(UUID id, UpdateCriteriaTemplateRequest req) {
        ...
        if (req.name() != null) {
            String name = req.name().trim();
            if (!name.equalsIgnoreCase(t.getName()) && repository.existsByNameIgnoreCase(name))
                throw ApiException.conflict("Criteria template name already exists");
            t.setName(name);
        }
     ...
     */
    private CriteriaTemplate find(UUID id) {
        return repository.findById(id).orElseThrow(() -> ApiException.notFound("Criteria template not found: " + id));
    }
