...
import vn.edu.fpt.seal.modules.judge.dto.AssignTrackJudgeRequest;
...
public class TrackJudgeService {
    ...
    public Page<TrackJudgeResponse> list(UUID trackId, UUID userId, Pageable pageable, Authentication authentication) {
        ...
        if (trackId != null)
            return trackJudgeRepository.findByTrackId(trackId, pageable).map(TrackJudgeMapper::toResponse);
        if (userId != null)
            return trackJudgeRepository.findByUserId(userId, pageable).map(TrackJudgeMapper::toResponse);
    ...
    public TrackJudgeResponse assign(AssignTrackJudgeRequest req) {
        ...
        if (trackJudgeRepository.existsByTrackIdAndUserId(track.getId(), user.getId()))
            throw ApiException.conflict("Judge already assigned to this track");
    ...
    public void remove(UUID id) {
        trackJudgeRepository.delete(trackJudgeRepository.findById(id).orElseThrow(() -> ApiException.notFound("Track judge assignment not found: " + id)));
    }

    @Transactional
    public void removeByTrackAndUser(UUID trackId, UUID userId) {
        trackJudgeRepository.delete(trackJudgeRepository.findByTrackIdAndUserId(trackId, userId).orElseThrow(() -> ApiException.notFound("Track judge assignment not found")));
    }
}
