...
public class SubmissionController {
    ...
    public ResponseEntity<SubmissionResponse> get(@PathVariable UUID id, Authentication auth) {
        return ResponseEntity.ok(submissionService.get(id, auth));
    }

    @PostMapping
    @PreAuthorize("isAuthenticated()")
    @Operation(summary = "Create or update submission")
    public ResponseEntity<SubmissionResponse> submit(@Valid @RequestBody UpsertSubmissionRequest req, Authentication auth) {
        return ResponseEntity.ok(submissionService.submit(req, auth));
    }

    @PatchMapping("/{id}")
    @PreAuthorize("isAuthenticated()")
    @Operation(summary = "Update submission")
    public ResponseEntity<SubmissionResponse> update(@PathVariable UUID id, @Valid @RequestBody UpdateSubmissionRequest req, Authentication auth) {
        return ResponseEntity.ok(submissionService.update(id, req, auth));
    }
