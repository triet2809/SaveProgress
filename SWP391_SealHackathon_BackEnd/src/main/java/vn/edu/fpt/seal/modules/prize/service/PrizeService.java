...
public class PrizeService {
    ...
    @Transactional(readOnly = true)
    public PrizeResponse get(UUID id) {
        return PrizeMapper.toResponse(find(id));
    }
    ...
    public PrizeResponse update(UUID id, UpdatePrizeRequest r) {
        Prize p = find(id);
        lifecycleService.requireAwardsAllowed(p.getEvent().getId());
        seedingService.requireEventFinalized(p.getEvent().getId());
    ...
    private Prize find(UUID id) {
        return repo.findWithRelationsById(id).orElseThrow(() -> ApiException.notFound("Prize not found: " + id));
    }

    private Track track(UUID id) {
        return id == null ? null : trackRepo.findById(id).orElseThrow(() -> ApiException.notFound("Track not found: " + id));
    }

    private Team team(UUID id) {
        return id == null ? null : teamRepo.findById(id).orElseThrow(() -> ApiException.notFound("Team not found: " + id));
    }

    private void validateScope(Event e, Track tr, Team tm) {
        if (tr != null && !tr.getEvent().getId().equals(e.getId()))
            throw ApiException.badRequest("Track must belong to prize event");
        if (tm != null) {
            if (tr != null && !tm.getTrack().getId().equals(tr.getId()))
                throw ApiException.badRequest("Team must belong to prize track");
            if (!tm.getTrack().getEvent().getId().equals(e.getId()))
                throw ApiException.badRequest("Team must belong to prize event");
        }
    }
