...
public class RoundRanking {
    ...
    @Column(name = "total_score", precision = 10, scale = 2) private BigDecimal totalScore;
    @Column(name = "rank") private Integer rank;
    @Enumerated(EnumType.STRING) @JdbcTypeCode(SqlTypes.NAMED_ENUM) @Column(name = "status", nullable = false, columnDefinition = "promotion_status") @Builder.Default private PromotionStatus status = PromotionStatus.pending;
    @ManyToOne(fetch = FetchType.LAZY) @JoinColumn(name = "tie_breaker_criterion_id") private RoundCriterion tieBreakerCriterion;
    @Column(name = "tie_breaker_score", precision = 10, scale = 2) private BigDecimal tieBreakerScore;
    @Column(name = "tie_breaker_reason", columnDefinition = "text") private String tieBreakerReason;
    @CreationTimestamp @Column(name = "calculated_at", nullable = false, updatable = false) private LocalDateTime calculatedAt;
