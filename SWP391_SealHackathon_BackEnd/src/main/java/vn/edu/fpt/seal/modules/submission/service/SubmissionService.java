...
public class SubmissionService {
    ...
    public Page<SubmissionResponse> list(UUID eventId, UUID roundId, UUID teamId, UUID trackId, Pageable pageable, Authentication auth) {
        ...
        if (roundId != null) {
            ...
            if (!round.getTrack().getEvent().getId().equals(scopedEventId)) throw ApiException.badRequest("Round does not belong to the selected event");
            if (trackId != null && !round.getTrack().getId().equals(trackId)) throw ApiException.badRequest("Round and track do not match");
        }
